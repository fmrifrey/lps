...
%
    ...
    kmsk_in = 2*rec_args.fov/seq_args.N_nom * vecnorm(reshape(ktraj_in(:,1,:),[],3),2,2) ...
        <= min(rec_args.N/seq_args.N_nom, (rec_args.fermi_cutoff + rec_args.fermi_rolloff));
    kmsk_out = 2*rec_args.fov/seq_args.N_nom * vecnorm(reshape(ktraj_out(:,1,:),[],3),2,2) ...
        <= min(rec_args.N/seq_args.N_nom, (rec_args.fermi_cutoff + rec_args.fermi_rolloff));
    ...
    for ivol = 1:nvol % loop through volumes and assemble nufft object for current vol
        omegav_in = 2*pi*rec_args.fov/rec_args.N * ...
            reshape(ktraj_in(kmsk_in,ivol,:),[],3);
        omegav_out = 2*pi*rec_args.fov/rec_args.N * ...
    ...
    ferm_omega = @(omega) ferm_fun(rec_args.N/rec_args.fov*seq_args.fov/seq_args.N_nom/pi * vecnorm(omega,2,2), ...
        2*rec_args.fermi_cutoff, ...
        rec_args.fermi_rolloff);
