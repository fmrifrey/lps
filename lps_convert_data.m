function [kdata,k_in,k_out,seq_args] = lps_convert_data(vendor, infile, h5file)
% gets data from scanarchive (ge) or twix (siemens) file, and formats the
% kspace trajectories and sequence arguments based on seq_args.h5, then 
% writes formatted data to h5 file for external recon if specified
% by David Frey (djfrey@umich.edu)
%
% inputs:
% vendor - 'ge' or 'siemens'
% infile - name of data file to read in
...
%
    ...
    switch lower(vendor)
        ...
        case 'siemens'
            twix = mapVBVD([sadir,'/',infile]);
            data = twix{2}.image.unsorted; % n x nc x nint*nprj*nrep+1
            [ndat,nc,~] = size(data);
            data = permute(data, [1, 3, 2]); % n x nint*nprj*nrep+1 x nc
            kdata = reshape(data(:,1:seq_args.nint*seq_args.nprj*seq_args.nrep,:), ...
